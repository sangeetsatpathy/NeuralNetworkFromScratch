...
def main():
    img_1 = load_mnist()[0][0]
    net = train_epoch()
    
    print("Finished training 0 epoch!")
    net.export_nn(f"eepoch_0.txt")
    accuracy = test_nn(net) # overflow error here!
    print(f"Epoch 0 accuracy: {accuracy}")
    print(f"Testing predictions on image 1...{net.image_inference(img_1[0])}")
    for e in range(1, 6):
        net = train_epoch(net)
        print(f"Finished training {e} epoch!")
        net.export_nn(f"eepoch_{e}.txt")
